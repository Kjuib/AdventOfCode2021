...
function step(state) {
    if (state.cost >= minCost) {
        ...
        minCost = state.cost;
        console.log('FOUND', minCost, stateMap.size);
        return;
    }
    ...
    _.forEach(state.hall, (hallSpot, hallIndex) => {
        ...
        let targetRoom = null;
        if (state.rooms[homeIndex][0] === '.' && state.rooms[homeIndex][1] === '.') {
            targetRoom = 1;
        } else if (state.rooms[homeIndex][0] === '.' && state.rooms[homeIndex][1] === char) {
            targetRoom = 0;
        }
    ...
    _.forEach(state.rooms, (roomList, homeIndex) => {
        if (
            (roomList[0] === inverseHomes[homeIndex] && roomList[1] === inverseHomes[homeIndex]) ||
            (roomList[0] === '.' && roomList[1] === inverseHomes[homeIndex]) ||
            (roomList[0] === '.' && roomList[1] === '.')
        ) {
            return;
        }
...
function main() {
    ...
    // const rooms = [ ['B', 'A'], ['C', 'D'], ['B', 'C'], ['D', 'A'] ];
    const rooms = [ ['C', 'B'], ['A', 'A'], ['B', 'D'], ['D', 'C'] ];
